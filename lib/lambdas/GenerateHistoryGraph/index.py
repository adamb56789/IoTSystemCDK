...
def add_plot(response, device_name, time_mode, show_absolute_humidity):
    ...
    if len(rows) != 2:
        raise Exception("Received wrong number of measures from timestream: " + str(len(rows)))
    ...
    temperature = np.array([d['Value']['ScalarValue'] for d in rows[0]['Data'][1]['TimeSeriesValue']]).astype("single")
    humidity = np.array([d['Value']['ScalarValue'] for d in rows[1]['Data'][1]['TimeSeriesValue']]).astype("single")
    ...
    ax_t.plot_date(time[MOVAVG_RADIUS:-MOVAVG_RADIUS], moving_average(temperature, MOVAVG_RADIUS*2+1), "-", color=color_t, label="Temperature")
    ...
    if show_absolute_humidity == "True":
        ...
        ax_ah.plot_date(time, absolute_humidity, '-', color=color_ah, label="Absolute Humidity")
